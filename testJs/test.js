...
//Quant on click sur champ date on fait apparâitre le calendrier
...
function calInit(divId, btName, fieldId, classTable, classDay, classSel){
   ...
   // dateEl.addEventListener("click", calToogle(0));

  
   document.getElementById("champ_date2").addEventListener("click", function(){
		calToogle(0);
	});

	document.getElementById("champ_dateRetour").addEventListener("click", function(){
		calToogle(0);
	});

	
	// vÃ©rifie lexistance de divId et fieldId
	if(calDiv==undefined || fieldId==undefined) return 0;
	var h = "";
...
function calClick(dateStr, id){
	...
	field.value = dateArr[0]+' '+mois+' '+dateArr[2];
	document.getElementById('calendarWrap'+id).style.display = "block";
...
var jsSDPObj = Array();
var jsSDPId = 0;
var jsSDPMonthName = ['Janvier', 'Février', 'Mars', 'Avril', 'Mai', 'Juin', 'Juillet', 'Août', 'Septembre', 'Octobre', 'Novembre', 'Decembre'];